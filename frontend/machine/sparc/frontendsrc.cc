...
bool isRestore(ADDRESS hostPC);         // In machine/sparc/decoder.m
 ...
 *============================================================================*/
bool FrontEndSrc::processProc(ADDRESS address, UserProc* proc, std::ofstream &os,
...
void emitCopyPC(std::list<RTL*>* pRtls, ADDRESS uAddr)
{
    // Emit %o7 = %pc
    Type dflt;
    TypedExp* a = new TypedExp(dflt, new Binary(opAssign,
...
void appendAssignment(Exp* lhs, Exp* rhs, int size, LOC_TYPE ltype,
  ADDRESS addr, std::list<RTL*>* lrtl)
{
    Type ty(ltype, size);
    TypedExp* a = new TypedExp(ty, new Binary(opAssign, lhs, rhs));
...
void quadOperation(ADDRESS addr, std::list<RTL*>* lrtl, OPER op)
{
    ...
    appendAssignment(lhs, rhs, 128, FLOATP, addr, lrtl);
}
...
//      rhs->push(idRegOf); rhs->push(idIntConst); rhs->push(8);
    ...
    Type dflt;
    TypedExp* a = new TypedExp(dflt, new Binary(opAssign, lhs, rhs));
    // Create an RTL with this one Exp
    std::list<Exp*>* lrt = new std::list<Exp*>;
    lrt->push_back(a);
    RTL* rtl = new RTL(addr, lrt);
...
void gen32op32gives64(OPER op, std::list<RTL*>* lrtl, ADDRESS addr) {
    ...
    Type tlong(INTEGER, 64);
    Exp* a = new TypedExp(tlong, new Binary(opAssign,
    ...
    Type dflt;
    a = new TypedExp(dflt, new Binary(opAssign,
        new Unary(opRegOf, new Const(8)),
...
//      rhs->push(idRegOf); rhs->push(idIntConst); rhs->push(8);
    ...
    appendAssignment(lhs, rhs, 32, INTEGER, addr, lrtl);
    return true;
}
