...
/***************************************************************************/ /**
  ...
  *                         ...                  restore                ...
  *
  *                    The restore instruction in ProcB will effectively set %o7 to be %i7, the address to which ProcB
  *will
  *                    return. So in effect ProcC will return to ProcA at the position just after the call to ProcB.
  *This
...
/***************************************************************************/ /**
  ...
  * PARAMETERS:        newBB - the new basic block delimited by the branch instruction. May be nullptr if this block has
  *been
  *                    built before.
...
#define BRANCH_DS_ERROR 0 // If set, a branch to the delay slot of a delayed
            ...
            case SD: {
                ...
                case NCT: {
                    ...
                    if (last->getKind() == STMT_CALL) {
                        ...
                        case_SD(address, sec_iface->getTextDelta(), sec_iface->getLimitTextHigh(), inst, delay_inst,
            ...
            case DD: {
                ...
                case NOP:
                case NCT: {
                    sequentialDecode = case_DD(address, sec_iface->getTextDelta(), inst, delay_inst, BB_rtls,
            ...
            case SCD: {
                ...
                default:
                    if (delay_inst.rtl->back()->getKind() == STMT_CALL) {
                        ...
                        sequentialDecode = case_SCD(address, sec_iface->getTextDelta(), sec_iface->getLimitTextHigh(),
            ...
            case SCDAN: {
                ...
                case NCT: {
                    sequentialDecode = case_SCDAN(address, sec_iface->getTextDelta(), sec_iface->getLimitTextHigh(),
                                                  inst, delay_inst, BB_rtls, cfg, targetQueue);
                    break;
                }
