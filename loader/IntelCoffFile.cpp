...
bool IntelCoffFile::RealLoad(const char *sName) {
    ...
    if (static_cast<signed long>(sizeof *psh * m_Header.coff_sections) !=
        read(m_fd, psh, sizeof *psh * m_Header.coff_sections)) {
        free(psh);
        return false;
    }
    ...
    if (!pSymbols)
        return false;
    if (static_cast<signed long>(m_Header.coff_num_syment * sizeof(struct coff_symbol)) !=
        read(m_fd, pSymbols, m_Header.coff_num_syment * sizeof(struct coff_symbol)))
        return false;
        ...
        if (!(pSymbols[iSym].csym_loadclass & 0x60) && (pSymbols[iSym].csym_sectnum <= m_Header.coff_sections)) {
            if (pSymbols[iSym].csym_sectnum > 0) {
                ...
                if (pSymbols[iSym].csym_type & 0x20) {
                    ...
                    assert(false); //    pSymbols[iSym].csym_value = ield_1C->SetName(var_8, 0, this,
                                   //    field_4[var_4].csym_value);
                else {
                    pSymbols[iSym].csym_value = fakeForImport; // TODO: external reference
                    fakeForImport -= 0x10000;
                    m_Symbols.Add(pSymbols[iSym].csym_value, name);
    ...
    for (int iSection = 0; iSection < m_Header.coff_sections; iSection++) {
        ...
        struct struct_coff_rel *pRel =
            (struct struct_coff_rel *)malloc(sizeof(struct struct_coff_rel) * psh[iSection].sch_nreloc);
        if (!pRel)
            return false;

        if (static_cast<signed long>(sizeof(struct struct_coff_rel) * psh[iSection].sch_nreloc) !=
            read(m_fd, pRel, sizeof(struct struct_coff_rel) * psh[iSection].sch_nreloc))
            return false;
        ...
        for (int iReloc = 0; iReloc < psh[iSection].sch_nreloc; iReloc++) {
            ...
            // printf("Relocating at %08lx: type %d, dest %08lx\n", tRel->r_vaddr + psi->uNativeAddr +
            // psh[iSection].sch_virtaddr, (int)tRel->r_type, pSym->csym_value);
            ...
            case RELOC_ADDR32 + 1:
                ...
                // psh[iSection].sch_virtaddr, pSym->csym_value);
            ...
            case RELOC_REL32:
                ...
                *pPatch += pSym->csym_value -
                           (unsigned long)(tRel->r_vaddr + psi->uNativeAddr + psh[iSection].sch_virtaddr + 4);
...
void IntelCoffFile::Close() { printf("IntelCoffFile::Close called\n"); }

LOAD_FMT IntelCoffFile::GetFormat() const { return LOADFMT_COFF; }

MACHINE IntelCoffFile::GetMachine() const { return MACHINE_PENTIUM; }

const char *IntelCoffFile::getFilename() const { return m_pFilename; }
...
const char *IntelCoffFile::SymbolByAddress(const ADDRESS dwAddr) { return m_Symbols.find(dwAddr); }
...
std::map<ADDRESS, std::string> &IntelCoffFile::getSymbols() { return m_Symbols.getAll(); }
...
int IntelCoffFile::readNative2(ADDRESS a) { return readNative(a, 2); }

int IntelCoffFile::readNative1(ADDRESS a) { return readNative(a, 1); }
