...
public:
    ...
    virtual bool visit(const std::shared_ptr<CallStatement> &stmt, bool &visitChildren) override;

    /// \copydoc StmtExpVisitor::visit
    /// Only consider the first return when final
    virtual bool visit(const std::shared_ptr<ReturnStatement> &stmt, bool &visitChildren) override;
