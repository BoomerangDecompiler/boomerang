...
/**
 * Test an address expression (operand of a memOf) for primitiveness (i.e. if it is possible to SSA
 * rename the memOf without problems). Note that the PrimitiveTester is not used with the memOf
 * expression, only its address expression
 */
...
public:
    ...
    // Return true if e is a primitive expression; basically, an expression you can propagate to
    // without causing memory expression problems. See Mike's thesis for details Algorithm: if find
    // any unsubscripted location, not primitive
