            ...
            case 18:
                ...
                if ((MATCH_w_32_0 >> 1 & 0x1) == 1) {
                    ...
                    MATCH_name = MATCH_name_LK_8[(MATCH_w_32_0 & 0x1)];
                    {
                        ...
                        Function *destProc = m_prog->createProc(Address(reladdr.value() - delta));
...
MATCH_label_a12:
        (void)0; /*placeholder for label*/
        {
            ...
            default:
                ...
                LOG_STREAM_OLD(LogLevel::Warning) << "ERROR: MTSPR instruction with invalid S field: " << uimm << "\n";
            }
...
PPCDecoder::PPCDecoder(Prog *_prog)
    : NJMCDecoder(_prog)
{
    m_rtlDict.readSSLFile(Boomerang::get()->getDataDirectory().absoluteFilePath("frontend/machine/ppc/ppc.ssl"));
}
...
SharedExp crBit(int bitNum)
{
    ...
    bitNum = bitNum & 3;
    return std::make_shared<Ternary>(opAt, Location::regOf(64 + crNum), Const::get(bitNum), Const::get(bitNum));
}
